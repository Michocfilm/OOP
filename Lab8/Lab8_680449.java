...
public class Lab8_680449 {
    ...
    static void q2_9() {
        ...
        // your implementation
        PriorityQueue<Employee> pqDesc = new PriorityQueue<>((e1, e2) -> Integer.compare(e2.getSalary(), e1.getSalary()));
        pqDesc.addAll(empList);

        for (int i = 0; i < 5; i++) {
            fiveTopSalary.add(pqDesc.poll());
        }
    ...
    static void q3_2_sub(int n, String[] inp) {
        ...
        for (String cmd : inp) {
            if (cmd.startsWith("VISIT")) {
                stack.add(cmd.split("\\s+")[1]);
            } else {
                if (stack.isEmpty()) {
                    System.out.println("Empty (Nothing to BACK)");
                } else {
                    System.out.println("Back to: " + stack.remove(stack.size() - 1));
                }
            }
        }
    }
    ...
    static void q3_3_sub(int n, String[] inp) {
        ...
        for (String cmd : inp) {
            if (cmd.startsWith("ADD")) {
                ...
                pq.add(new Task(t[1], Integer.parseInt(t[2]), order++));
            } else {
                if (pq.isEmpty()) {
    ...
    static void q3_4_sub(String inp) {
        ...
        System.out.println(set.size() % 2 == 0 ? "Girl" : "Boy");
    }
